...
// Masquer la section de mise à jour du thème Mediapilote sur update-core.php
add_action('admin_head-update-core.php', function() {
    $theme = wp_get_theme();
    $theme_name = esc_js($theme->get('Name'));
    ?>
    <style>
    form[name="upgrade-themes"]{
    ...
    <script>
    // Pour plus de robustesse, masquer dynamiquement côté JS
    document.addEventListener('DOMContentLoaded', function() {
        var themeName = <?php echo json_encode($theme_name); ?>;
        var h2s = document.querySelectorAll('#themes-to-update h2');
        h2s.forEach(function(h2) {
            if (h2.textContent.trim().indexOf(themeName) !== -1) {
                var p = h2.nextElementSibling;
                var form = p ? p.nextElementSibling : null;
                h2.style.display = 'none';
                if (p && p.tagName === 'P') p.style.display = 'none';
                if (form && form.classList.contains('upgrade')) form.style.display = 'none';
            }
        });
    });
    </script>
    <?php
// Fin du hook admin_head-update-core.php
...
// Ajout de la page d'admin pour vérifier la version du thème
require_once get_template_directory() . '/inc/version-checker.php';
// Ajout de la page d'admin pour la mise à jour automatique
